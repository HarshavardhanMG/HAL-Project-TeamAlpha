...
class DocumentProcessor:
    ...
    def initialize_apis(self):
        models = []
        generation_config = {
    "temperature": 0.7,   # Moderate randomness for balance
    "top_p": 1.0,         # Use the full probability distribution (no truncation)
    "top_k": 50,          # Consider a wide range of tokens
    "max_output_tokens": 10240,  # Increase token limit for more comprehensive feedback
}
...
def main():
    ...
    # Update base directory to be relative to the script's location
    base_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(base_dir, "results")
    os.makedirs(output_dir, exist_ok=True)
    
    def process_student(i):
        student_file = os.path.join(base_dir, f"stu_{i}.txt")
        output_file = os.path.join(output_dir, f"stu_{i}_results.txt")
        
        result_path = processor.process_documents(
            os.path.join(base_dir, "question_paper.txt"),
            os.path.join(base_dir, "answer_key.txt"),
            student_file,
            output_file
        )
        return result_path
