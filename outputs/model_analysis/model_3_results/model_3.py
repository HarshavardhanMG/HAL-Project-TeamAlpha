...
class DocumentProcessor:
    ...
    def initialize_apis(self):
        models = []
        generation_config = {
    "temperature": 1.0,   # Higher randomness for creative outputs
    "top_p": 0.95,        # Wider sampling distribution
    "top_k": 60,          # Broader token consideration for richer responses
...
def main():
    ...
    question_paper_path = os.path.join(script_dir, "question_paper.txt")
    answer_key_path = os.path.join(script_dir, "answer_key.txt")
    ...
    def process_student(i):
        student_file = os.path.join(script_dir, f"stu_{i}.txt")
