...
class RateLimiter:
    ...
    def can_make_request(self, api_id):
        ...
        with self.locks[api_id]:
            ...
            if (len(self.request_timestamps[api_id]) >= 12 or  # 60 requests/min divided by 5 APIs
                self.daily_counts[api_id]['count'] >= 300):    # 1500 daily divided by 5 APIs
                return False
...
class DocumentProcessor:
    ...
    "top_p": 0.8,         # Narrow probability mass to focus on top tokens
...
def main():
    ...
    output_dir = "./model_analysis/results"
    os.makedirs(output_dir, exist_ok=True)
    
    def process_student(i):
        student_file = f"./model_analysis/stu_{i}.txt"
        output_file = f"{output_dir}/stu_{i}_results.txt"
        
        result_path = processor.process_documents(
            "./model_analysis/question_paper.txt",
            "./model_analysis/answer_key.txt",
            student_file,
            output_file
        )
        return result_path
